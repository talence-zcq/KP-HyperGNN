...
from utils.data_utils import cal_the_normalization_diag_matrix
...
#             data.V2Eedge_index[0] contains nodes and data.V2Eedge_index[1] contains hyperedges
        ...
        N_node = x.shape[0]
        Hyper_edge = torch.unique(edge_index[1]).size(0)
        shape = torch.Size([N_node, Hyper_edge])
        values = torch.ones(edge_index.shape[1]).to(self.device)
        indices = edge_index
        induced_matrix = torch.sparse_coo_tensor(indices, values, shape)
        # Line_graph_induced_matrix = torch.sparse.mm(induced_matrix.transpose(0, 1), induced_matrix)
        # Line_graph_diag_matrix = cal_the_normalization_diag_matrix(Line_graph_induced_matrix)
        # Line_graph_induced_normalization_matrix = torch.sparse.mm(Line_graph_diag_matrix,
        #                                                           torch.sparse.mm(Line_graph_induced_matrix,
        #                                                                           Line_graph_diag_matrix))
        # Node_graph_induced_matrix = torch.sparse.mm(induced_matrix, induced_matrix.transpose(0, 1))
        # Node_graph_diag_matrix = cal_the_normalization_diag_matrix(Node_graph_induced_matrix)
        # Node_graph_induced_normalization_matrix = torch.sparse.mm(Node_graph_diag_matrix, torch.sparse.mm(Node_graph_induced_matrix, Node_graph_diag_matrix))
        #
        #
...
#                 x = self.bnE2Vs[i](x)
            ...
            for i, _ in enumerate(self.V2EConvs):
                x = F.relu(self.V2EConvs[i](x, edge_index, norm, self.aggr))
                # x = self.bnV2Es[i](x)
                # x = torch.sparse.mm(Line_graph_induced_normalization_matrix, x)
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = F.relu(self.E2VConvs[i](
                    x, reversed_edge_index, norm, self.aggr))
#                 x = self.bnE2Vs[i](x)
#                 x = torch.sparse.mm(Node_graph_induced_normalization_matrix, x)
                x = F.dropout(x, p=self.dropout, training=self.training)
            x = self.classifier(x)
...
class UniGCNIIConv(nn.Module):
    ...
    def forward(self, X, vertex, edges, alpha, beta, X0):
        ...
        # Xe = torch.sparse.mm(self.args.Line_graph_induced_matrix, Xe)

        Xe = Xe * degE 

        Xev = Xe[edges] # [nnz, C]
        Xv = scatter(Xev, vertex, dim=0, reduce='sum', dim_size=N) # [N, C]
        # Xv = torch.sparse.mm(self.args.Node_graph_induced_matrix, Xv)
